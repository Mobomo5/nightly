...
CREATE TABLE IF NOT EXISTS user (
  ...
  UNIQUE INDEX givenIdentifier_UNIQUE (givenIdentifier ASC),
...
CREATE TABLE IF NOT EXISTS userFriendship (
  ...
  INDEX fk_userFriendship_user_idx (userID ASC),
...
CREATE TABLE IF NOT EXISTS permissionSet (
  ...
  INDEX fk_permissionSet_role1_idx (roleID ASC),
  INDEX fk_permissionSet_permission1_idx (permissionID ASC),
  ...
  CONSTRAINT fk_permissionSet_permission1
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'Relates user role\'s to permissions and keeps track if the ro' /* comment truncated */ /*le is allowed to do the action specified in the permission.*/;
...
CREATE TABLE IF NOT EXISTS nodeType (
  ...
  UNIQUE INDEX nodeTypeID_UNIQUE (nodeTypeID ASC),
  INDEX fk_nodeType_module1_idx (module ASC),
...
CREATE TABLE IF NOT EXISTS status (
  ...
  INDEX fk_status_status1_idx (parentStatus ASC),
  INDEX fk_status_user1_idx (posterID ASC),
  INDEX fk_status_node1_idx (nodeID ASC),
...
CREATE TABLE IF NOT EXISTS statusSupporter (
  ...
  INDEX fk_statusSupporter_user1_idx (supporterID ASC),
  INDEX fk_statusSupporter_status1_idx (statusID ASC),
...
CREATE TABLE IF NOT EXISTS systemLog (
  ...
  INDEX fk_systemLog_user1_idx (userID ASC),
...
CREATE TABLE IF NOT EXISTS statusRevision (
  ...
  INDEX fk_statusRevision_status1_idx (statusID ASC),
  INDEX fk_statusRevision_user1_idx (revisorID ASC),
...
CREATE TABLE IF NOT EXISTS nodeFieldType (
  ...
  UNIQUE INDEX fieldName_UNIQUE (fieldName ASC))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS nodeFieldRevision (
  ...
  PRIMARY KEY (revisionID),
  UNIQUE INDEX revisionID_UNIQUE (revisionID ASC),
  INDEX fk_nodeRevision_user1_idx (authorID ASC),
  INDEX fk_nodeRevision_node1_idx (nodeID ASC),
  INDEX fk_nodeFieldRevision_nodeFieldType1_idx (nodeFieldType ASC),
  ...
  CONSTRAINT fk_nodeFieldRevision_nodeFieldType1
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'Tracks the edits of node fields. Also holds the main content' /* comment truncated */ /* of the node fields.*/;
...
CREATE TABLE IF NOT EXISTS userOption (
  ...
  UNIQUE INDEX optionName_UNIQUE (optionName ASC))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS userOptionSet (
  ...
  UNIQUE INDEX optionSetID_UNIQUE (optionSetID ASC),
  INDEX fk_userOptionSet_user1_idx (userID ASC),
  INDEX fk_userOptionSet_userOption1_idx (optionID ASC),
  ...
  CONSTRAINT fk_userOptionSet_userOption1
    ...
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'Relates a user to user options and keeps track on their pref' /* comment truncated */ /*erences for the option.*/;
...
CREATE TABLE IF NOT EXISTS urlAlias (
  ...
  PRIMARY KEY (aliasID),
  UNIQUE INDEX aliasID_UNIQUE (aliasID ASC))
ENGINE = InnoDB
COMMENT = 'Maps a non-pretty URL to a pretty URL. Example: node/35/cat/' /* comment truncated */ /*drtiou/325s/a to /elephant.*/;


CREATE TABLE IF NOT EXISTS nodeVisibility (
  ruleID INT NOT NULL AUTO_INCREMENT COMMENT 'A unique identifier for the rule',
  nodeID INT NOT NULL COMMENT 'The node the rule applies to.',
  ...
  INDEX fk_nodeVisibility_node1_idx (nodeID ASC),
  CONSTRAINT fk_nodeVisibility_node1
    FOREIGN KEY (nodeID)
    REFERENCES node (nodeID)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS activeDirectory (
  ...
  UNIQUE INDEX adUsername_UNIQUE (adUsername ASC),
  PRIMARY KEY (adUsername),
  INDEX fk_activeDirectory_user1_idx (userID ASC),
  UNIQUE INDEX userID_UNIQUE (userID ASC),
...
CREATE TABLE IF NOT EXISTS menuItem (
  ...
  INDEX fk_menuItem_menu1_idx (menuID ASC),
  INDEX fk_menuItem_menuItem1_idx (parent ASC),
...
CREATE TABLE IF NOT EXISTS menuItemVisibility (
  ...
  PRIMARY KEY (ruleID),
  UNIQUE INDEX ruleID_UNIQUE (ruleID ASC),
  INDEX fk_menuItemVisibility_menuItem1_idx (menuItemID ASC),
...
CREATE TABLE IF NOT EXISTS groupMember (
  ...
  INDEX fk_groupMember_user1_idx (userID ASC),
...
CREATE TABLE IF NOT EXISTS assignment (
  assignmentID INT NOT NULL AUTO_INCREMENT COMMENT 'The unique identifier for the assignment.',
  outOf INT NOT NULL COMMENT 'How much the assignment is out of.',
  weight DECIMAL(4,2) NOT NULL COMMENT 'How much the assignment contributes to the final grade.',
  dueDate TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'The time when the assignment is due.',
  assignmentNodeID INT NOT NULL COMMENT 'The node ID of the assignment.',
  classNodeID INT NOT NULL COMMENT 'The node that the assignment is posted to.',
  PRIMARY KEY (assignmentID),
  UNIQUE INDEX assignementID_UNIQUE (assignmentID ASC),
  INDEX fk_assignement_node1_idx (classNodeID ASC),
  INDEX fk_assignement_node2_idx (assignmentNodeID ASC),
  CONSTRAINT fk_assignement_node1
    FOREIGN KEY (classNodeID)
    REFERENCES node (nodeID)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT fk_assignement_node2
    FOREIGN KEY (assignmentNodeID)
    REFERENCES node (nodeID)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
COMMENT = 'Information about an assignment on a class page.';
...
CREATE TABLE IF NOT EXISTS assignmentMark (
  ...
  assignmentID INT NOT NULL COMMENT 'The assignment the mark is for.',
  userID INT NOT NULL COMMENT 'The user receiving the grade.',
  PRIMARY KEY (id),
  UNIQUE INDEX id_UNIQUE (id ASC),
  INDEX fk_assignmentMark_assignment1_idx (assignmentID ASC),
  INDEX fk_assignmentMark_user1_idx (userID ASC),
  CONSTRAINT fk_assignmentMark_assignment1
...
CREATE TABLE IF NOT EXISTS block (
  ...
  INDEX fk_block_module1_idx (module ASC),
...
CREATE TABLE IF NOT EXISTS blockVisibility (
  ...
  PRIMARY KEY (ruleID),
  UNIQUE INDEX ruleID_UNIQUE (ruleID ASC),
  INDEX fk_blockVisibility_block1_idx (blockID ASC),
...
CREATE TABLE IF NOT EXISTS folder (
  ...
  INDEX fk_folder_user1_idx (ownerID ASC),
  INDEX fk_folder_folder1_idx (parentFolder ASC),
...
CREATE TABLE IF NOT EXISTS file (
  ...
  INDEX fk_files_node1_idx (nodeID ASC),
  INDEX fk_files_user1_idx (uploader ASC),
  INDEX fk_files_folder1_idx (folderID ASC),
...
CREATE TABLE IF NOT EXISTS fileSystemShare (
  ...
  PRIMARY KEY (ruleID),
  UNIQUE INDEX ruleID_UNIQUE (ruleID ASC),
  INDEX fk_fileSystemShare_user1_idx (userID ASC),
...
CREATE TABLE IF NOT EXISTS forgotPassword (
  ...
  INDEX fk_forgotPassword_user1_idx (userID ASC),
  UNIQUE INDEX userID_UNIQUE (userID ASC),
...
CREATE TABLE IF NOT EXISTS censorship (
  ...
  UNIQUE INDEX censorID_UNIQUE (censorID ASC))
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS message (
  ...
  INDEX fk_message_status1_idx (statusID ASC),
  INDEX fk_message_user1_idx (senderID ASC),
  INDEX fk_message_node1_idx (nodeID ASC),
...
CREATE TABLE IF NOT EXISTS messageRecipient (
  ...
  PRIMARY KEY (id),
  INDEX fk_messageRecipient_message1_idx (messageID ASC),
  INDEX fk_messageRecipient_user1_idx (recipientID ASC),
  UNIQUE INDEX id_UNIQUE (id ASC),
